...
function pseudo_marginal_refractive_sampler(init_x::Vector{Float64},
                                            f::Function,
                                            grad_f::Function,
                                            particles,
...
#  end
  ...
  fx0= f(T(x0), particles) + T_logjacobian(x0)
  accept_logprob = 0.0


  for iteration = 1:m+1
    fx = f(T(x), particles) + T_logjacobian(x) 
    if !isfinite(fx)
        println("reject: out of bounds?")
        accept_logprob = -Inf
        break
    end
    grad = T_grad(x, grad_f(T(x), particles))

    grad_norm = sqrt(dot(grad,grad))
 
    grad = grad/grad_norm

    # update particles prior to every update to x
    accept_logprob += update_function!(x, particles)

    if verify_gradient
        epsilon=10.0^-6
        x_test = x + epsilon*grad
        fx_test = f(T(x_test), particles) + T_logjacobian(x_test)
  ...
  # one final update at the end for reversibility
  accept_logprob += update_function!(x, particles)
